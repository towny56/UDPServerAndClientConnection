...
public class PrintItemsets {
	...
	public static void print_itemsetRec(BufferedWriter f, itemsetRec is, boolean isCSVInputFile) throws IOException {
		print_itemset(f, is, isCSVInputFile);

		f.write(" [" + is.count + " , " +  is.value);
		f.write(" " + is.p + "]");
	...
	public static void print_itemsets(BufferedWriter f, ArrayList<itemsetRec> is, boolean isCSVInputFile) throws IOException {
		int i;

		for (i = 2; i < Global.alpha.size(); i++) {
			f.write("Alpha for size " + i + " " + Global.alpha.get(i) + '\n');
		}

		f.write("\nSELF-SUFFICIENT ITEMSETS:\n");

		// C++ TO JAVA CONVERTER TODO TASK: The 'Compare' parameter of std::sort
		...
		while (it.hasNext()) {
			...
			if (!itemsetRec.self_sufficient) {
				...
				print_itemsetRec(f,itemsetRec,isCSVInputFile);
			}
		}
